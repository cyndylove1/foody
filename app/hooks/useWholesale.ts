...
const fetchWholesale = async ({
  ...
  // Append page parameter (e.g., /products/retail?page=1)
  const fullUrl = `${BASE_URL}${baseEndpoint}?page=${pageParam}`;

  const response = await axios.get(fullUrl);
  const rawList = extractArray(response.data);

  const formattedProducts: Product[] = rawList.map((item: any) => ({
    id: item.id,
  ...
  // Determine if there is a next page based on standard pagination fields or list length
  const lastPageMeta = response.data?.meta || response.data?.data;
  const hasMore =
    lastPageMeta?.current_page < lastPageMeta?.last_page || rawList.length > 0; // If rawList is non-empty, assume next page exists (or tune based on page size)
